            ...
            <div v-if="loadClientList.length === 0">
                <q-item>
                    <q-item-section class="text-center">
                        <q-item-label>
                            <q-icon name="report" color="grey-3" size="3rem" /><br>
...
export default {
        ...
        filterList(){
            if(this.loadClientList.length !== 0){
                ...
                return this.loadClientList.filter(search => {
                    let filtered = "";
                    if(filterCatId === 0 && filterBranch === "All"){
                        filtered = search.client.storeName.toLowerCase().includes(this.searchClient.toLowerCase()); 
                    } else if(filterCatId === 0 && filterBranch !== "All") {
                        filtered = search.client.storeName.toLowerCase().includes(this.searchClient.toLowerCase()) && search.client.addressDetails.province.label === filterBranch;
                    } else if(filterCatId !== 0 && filterBranch === "All"){
                        filtered = search.client.storeName.toLowerCase().includes(this.searchClient.toLowerCase()) && search.client.categoryId === filterCatId;
                    } else {
                        filtered = search.client.storeName.toLowerCase().includes(this.searchClient.toLowerCase()) && 
    ...
    methods: {
        ...
        async getClientListPref(){
            ...
            api.post('mobile/fetch/order/list', { aid:Number(this.user.userId), orderDate: this.curDate })
            .then(async (response) => {
                console.log(response)
                this.loadClientList = response.list
            }).catch((err) => {
                this.loadClientList = []
            })
        },
